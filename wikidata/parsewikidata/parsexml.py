            ...
            for event, elem in etree.iterparse(pathWikiXML, events=('start', 'end')):
                if event == 'start':
                    ...
                    if elem.tag == '{http://www.mediawiki.org/xml/export-0.10/}page':
                            ...
                            if (pagetitle != '' and pageid != 0 and label!='' and revisionid != 0 and timestamp !='' and timestamp is not None and convert_date_time(timestamp) > d22) :
                                ...
                                if (comment is None):
                                   comment = ''
                                articlesWriter.writerow([pageid, pagetitle, label, revisionid, timestamp, (comment).encode('utf8'), parentid])
                                counter += 1
                                print(counter)
                            revisionid=0
                            inrevision = True
